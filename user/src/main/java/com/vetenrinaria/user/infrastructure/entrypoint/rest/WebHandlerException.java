...
import com.vetenrinaria.user.infrastructure.entrypoint.exceptions.TechnicalExceptions;
import com.vetenrinaria.user.infrastructure.entrypoint.exceptions.TechnicalMessageExceptions;
    ...
    private HttpStatus resolveHttpStatus(Throwable ex){
        if (ex instanceof BusinessExceptions businessExceptions){
            return switch (businessExceptions.getBusinessMessageExceptions()){
                case USER_INVALID , USER_EXIST, ROLE_INVALID -> HttpStatus.CONFLICT;
                case USER_NOT_EXIST -> HttpStatus.NOT_FOUND;
            };
