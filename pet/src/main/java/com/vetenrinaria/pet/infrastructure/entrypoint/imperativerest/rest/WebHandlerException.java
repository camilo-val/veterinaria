...
import com.vetenrinaria.pet.infrastructure.entrypoint.imperativerest.exceptions.TechnicalExceptions;
import com.vetenrinaria.pet.infrastructure.entrypoint.imperativerest.exceptions.TechnicalMessageExceptions;
...
public class WebHandlerException implements HandlerFilterFunction<ServerResponse, ServerResponse> {
    ...
    private HttpStatus resolveHttpStatus(Throwable ex){
        if (ex instanceof com.vetenrinaria.pet.domain.model.exceptions.BusinessExceptions businessExceptions){
            return switch (businessExceptions.getBusinessMessageExceptions()){
                case PET_EXIST, PET_INVALID -> HttpStatus.CONFLICT;
                case PET_NOT_EXIST -> HttpStatus.NOT_FOUND;
            };
