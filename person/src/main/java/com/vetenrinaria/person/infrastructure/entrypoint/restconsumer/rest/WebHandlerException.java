...
import com.vetenrinaria.person.infrastructure.entrypoint.restconsumer.exceptions.TechnicalExceptions;
import com.vetenrinaria.person.infrastructure.entrypoint.restconsumer.exceptions.TechnicalMessageExceptions;
    ...
    private HttpStatus resolveHttpStatus(Throwable ex){
        if (ex instanceof BusinessExceptions businessExceptions){
            return switch (businessExceptions.getBusinessMessageExceptions()){
                case PERSON_INVALID , PERSON_EXIST -> HttpStatus.CONFLICT;
                case PERSON_NOT_EXIST -> HttpStatus.NOT_FOUND;
            };
